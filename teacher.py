...
def teacher_post():
	...
	if 'submit' in request.form:		
		...
		file.save(path)
		q="insert into post_notes values(NULL,'%s','%s','%s')"%(shid,tid,path)
		insert(q)
		return redirect(url_for('teacher.teacher_post',shid=shid))
	q="select * from post_notes where schedule_id='%s'"%(shid)
	res=select(q)
	if res:
		data['notes']=res
		print(res)
...
def teacher_view_doubts():
	...
	data['shid']=shid
	tid=session['tid']
	q="SELECT * FROM doubts INNER JOIN students USING(student_id) where schedule_id='%s'"%(shid)
	res=select(q)
	data['doubt']=res
	print(res)
	if 'action' in request.args:
